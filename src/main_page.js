...
function append_nodes_relation (relations_data) {

}

// 获取节点
function get_expand_nodes(node_data) {
  console.log('node_data')
  console.log(node_data)
  $.ajax({
    async: true,
    url: 'http:// bigcode.fudan.edu.cn/kg-debug/yaml/expandNode/',
    type: 'post',
    ...
    success: function(d) {
      ...
      labelsSet = new Set(labels)
      console.log('labelsSet', labelsSet)
      for (var value of labelsSet) {
        var color_label = {}
        colorIndex = allLabelsColor.indexOf(value)
        console.log('color:')
        console.log(allLabelsColor)
...
function onNodeDoubleClick (d) {
  ...
  let dx = d.x
  let dy = d.y
  let parametersJson = { id: nodeID, label_set: graph_type, limit: 30 }
  ...
  $.ajax({
    ...
    success: function(d) {
      ...
      var labels = []
      $.each(d.nodes, function(i, val) {
        if (labelsSet.has(d.nodes[i]['labels'][0]) == false) {
          labelsSet.add(d.nodes[i]['labels'][0])
          var color_label = {}
          colorIndex = allLabelsColor.indexOf(d.nodes[i]['labels'][0])
          color_label['color'] = generateRandomColor(colorIndex)
...
function onNodeClick (d) {
  ...
  let nodeID = d.id
  let hide =
    "<b><i onclick=hide_flow_panel()><span class='glyphicon glyphicon-arrow-right'></span>Click to hide</i></b><br>"
...
function get_all_labels() {
  $.ajax({
    async: true,
    url: 'http:// bigcode.fudan.edu.cn/kg-debug/yaml/GetAllLabels/',
    type: 'post',
    contentType: 'application/json charset=utf-8',
...
function deletaNode () {
  ...
  $('#flowPanel').html(
    '<span>You can cilck the node to know about the information about it.<br>The information you want to know will display here.</span>'
  )
}
  ...
  for (let i = 0; i < labelStatusList.length; i++) {
    for (let j = 0; j < tempNR.length; j++) {
      for (let k = 0; k < tempNR[j].relationships.length; k++) {
        ...
        if (
          labelStatusList[i].status === false &&
          (labelStatusList[i].name === source.labels[0] ||
            labelStatusList[i].name === target.labels[0])
        ) {
  ...
  for (let i = 0; i < labelStatusList.length; i++) {
    for (let j = 0; j < tempNR.length; j++) {
      for (let k = 0; k < tempNR[j].nodes.length; k++) {
        ...
        if (
          labelStatusList[i].status === false &&
          labelStatusList[i].name === labels[0]
        ) {
...
function getRelationTypes(nodesAndRelations) {
  ...
  for (let i = 0; i < nodesAndRelations.length; i++) {
    for (let j = 0; j < nodesAndRelations[i].relationships.length; j++) {
      if (
        $.inArray(nodesAndRelations[i].relationships[j].type, type_list) === -1
      ) {
...
function getNodeLabels(nodesAndRelations) {
  ...
  for (let i = 0; i < nodesAndRelations.length; i++) {
    for (let j = 0; j < nodesAndRelations[i].relationships.length; j++) {
      for (
        let k = 0;
        k < nodesAndRelations[i].relationships[j].source.labels.length;
        k++
      ) {
        if (
          $.inArray(
            nodesAndRelations[i].relationships[j].source.labels[k],
            label_list
          ) === -1
        ) {
          label_list.push(
      ...
      for (
        let k = 0;
        k < nodesAndRelations[i].relationships[j].target.labels.length;
        k++
      ) {
        if (
          $.inArray(
            nodesAndRelations[i].relationships[j].target.labels[k],
            label_list
          ) === -1
        ) {
          label_list.push(
